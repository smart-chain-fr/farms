...
let initialize(storage : storage_farm) : return =
    ...
    let _check_if_unitialized : unit = assert_with_error (List.size storage.reward_at_week <> 0n) contract_already_initialized in
...
let claim_all(storage : storage_farm) : return = 
    ...
    match Big_map.find_opt sender_address user_points with
            ...
            | Some(user_points) ->
                ...
                let send_reward : operation = sendReward(total_reward_for_user, sender_address, storage) in// TODO storage a enlever
